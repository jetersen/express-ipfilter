...
module.exports = function ipfilter(ips, opts) {
  ...
  const stripPort = (ip) => {
    if (ip.startsWith('[')) {
      // Bracketed IPv6 with port: [::1]:3000 -> ::1
      const bracket = ip.indexOf(']')
      return bracket > -1 ? ip.substring(1, bracket) : ip
    }
    if (iputil.isV4Format(ip) || iputil.isV6Format(ip)) {
      return ip
    }
    // Unrecognized format, try stripping port from the end
    const lastColon = ip.lastIndexOf(':')
    return lastColon > -1 ? ip.substring(0, lastColon) : ip
  }
